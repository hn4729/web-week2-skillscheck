...
let bigOrSmall = arr => {
	let answers = [];
	arr.forEach(value => value > 100 ? answers.push("big") : answers.push("small"));
	return answers;
};
...
let total = myNumbers.reduce((total, val) => total + val);
...
let forTheLoveOfGeorge = notGeorge.map(value => "George");
...
let enemies = people.filter(value => !value.friend);
...
let totallyAwesome = people.reduce(function(total, value) {return total + value.awesomeLevel}, 0);
